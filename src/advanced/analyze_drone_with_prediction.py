...
def cluster_propeller_centers(
    ...
    Returns:
        Tuple of (list of (center_x, center_y) tuples, list of event counts per cluster)
    """
    ...
    propeller_centers = [(float(cx), float(cy)) for cx, cy in centers]
...
def assign_events_to_propellers(
    ...
    """Assign events to individual propellers based on proximity to centers.
    
    Args:
        x_coords: X coordinates of events
        y_coords: Y coordinates of events
        timestamps: Timestamps of events
        polarities: Polarities of events
        propeller_centers: List of (x, y) propeller centers
        max_distance: Maximum distance to assign event to propeller
        
    Returns:
        List of (x, y, t, pol) tuples for each propeller
    """
    n_propellers = len(propeller_centers)
    if n_propellers == 0 or len(x_coords) == 0:
        return [(np.array([]), np.array([]), np.array([]), np.array([])) 
                for _ in range(4)]
    
    # Compute distances from each event to each propeller center
...
def main() -> None:
    ...
    parser.add_argument(
        "bbox",
        type=Path,
        help="Path to bounding box detection file",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Maximum time gap for bbox interpolation in seconds (default: 0.5s)",
    )
    parser.add_argument(
        "--denoise",
        action="store_true",
    ...
    print(f"Bounding box file: {args.bbox}")
    ...
    try:
        for batch_range in pacer.pace(src.ranges()):
            ...
            if bbox:
                ...
                # Drone lost - reset RPM estimators to avoid stale values
                if rpm_estimator.last_valid_rpm > 0:
                    # Decay the last valid RPM slowly
                    rpm_estimator.last_valid_rpm *= 0.95
