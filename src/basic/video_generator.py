...
def compute_rotor_and_drone_bboxes(
   highlight_frame: np.ndarray,
    ...
    try:
        ...
        # On DBSCAN error, just return no boxes
        return [], None
...
class BoxKalmanTracker:
    ...
    def predict(self, t: float):
        ...
        if len(self.recent_states) > 200:
            self.recent_states = self.recent_states[-200:]
...
def draw_future_gaussian_distribution(
    ...
    """
    if not getattr(tracker, "initialized", False) or not GUASS:
        return frame
...
def draw_drone_hud(
    ...
    rpm: float,
    *,
    tracker: BoxKalmanTracker | None = None,
    predicted_box: tuple[int, int, int, int] | None = None,
    hud_color: tuple[int, int, int] = (255, 255, 255),        # text color (BGR)
    rotor_color: tuple[int, int, int] = (0, 0, 0),    # propellers (green)
    drone_color: tuple[int, int, int] = (0, 255, 0),    # measurement drone (red)
    pred_color: tuple[int, int, int] = (255, 0, 0),     # predicted box (blue)
    traj_color: tuple[int, int, int] = (0, 255, 0),   # trajectory path (yellow)
) -> None:
    """
    Visualization using the same style as your draw_hud template:
      ...
      - Yellow polyline: trajectory path of recent centers
    """
    # ---------------- HUD PART ----------------
    ...
    if drone_box is not None and DRONE_BOX:
        x, y, w, h = drone_box
        cv2.rectangle(
            frame,
            (int(x), int(y)),
    ...
    # Trajectory path from Kalman tracker (recent centers)
    if tracker is not None and getattr(tracker, "recent_states", None):
        pts = []
        for (_, cx, cy) in tracker.recent_states:
            pts.append((int(round(cx)), int(round(cy))))

        if len(pts) >= 2 and TRAJECTROY:
            pts_np = np.array(pts, dtype=np.int32).reshape((-1, 1, 2))
            cv2.polylines(frame, [pts_np], isClosed=False, color=traj_color, thickness=2)


# ==============================================================================
# 6. VIDEO WRITER HELPER
# ==============================================================================

def create_video_writer(path: str, fps: float, frame_size: tuple[int, int]) -> cv2.VideoWriter | None:
    """
    Try several codecs to create a VideoWriter.
    Returns an opened cv2.VideoWriter or None.
    """
    # Try true H.264 first, then some common fallbacks.
    codec_candidates = ["H264", "avc1", "X264", "mp4v"]
...
# ==============================================================================
# 7. MAIN CONTINUOUS LOGIC WITH REAL-TIME VISUALIZATION
# ==============================================================================
...
def main_frequency_estimation() -> None:
    ...
    # Output H.264 (or fallback) video
    parser.add_argument(
        "--output",
        type=str,
        default=None,
    ...
    try:
        ...
        # VideoWriter (lazy init on first rendered frame)
        video_writer = None
        # Approximate FPS from window size (e.g. 10ms window -> 100 fps)
        fps = 1000.0 / args.window_ms if args.window_ms > 0 else 30.0
        ...
        for i, (start_idx, stop_idx) in enumerate(time_windows):
            ...
            # Print only Dominant RPM (one line per processed window)
            #"print(f"{rpm:.2f}")
            ...
            if max_v > 0:
                vis_norm = highlight_frame / max_v
            else:
                vis_norm = highlight_frame

            # Gray baseline (128) + brighter where vis_norm is high
            background_gray = 0.0
            vis_u8 = np.clip(background_gray + vis_norm * 127.0, 0, 255).astype(np.uint8)

            # Build frame from this gray map
            frame = cv2.cvtColor(vis_u8, cv2.COLOR_GRAY2BGR)
            ...
            # Draw HUD + bounding boxes + predicted box + trajectory path
            draw_drone_hud(
                frame,
                pacer=pacer,
            ...
            if args.output is not None and video_writer is None:
                frame_size = (frame_gauss.shape[1], frame_gauss.shape[0])  # (width, height)
                video_writer = create_video_writer(args.output, fps, frame_size)

            # Write frame to video if writer is ready
            if video_writer is not None:
                video_writer.write(frame_gauss)
        ...
        # Release writer and close windows
        if video_writer is not None:
